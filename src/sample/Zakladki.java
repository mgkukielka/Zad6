...
public class Zakladki {
    ...
    private ObservableList<Czlowieczek> tabela_dane = FXCollections.observableArrayList();
    ...
    public void dane(ActionEvent actionEvent) {
        ...
        try {
            ...
            cntr.setParentController(this);
            cntr.dodawajDane();
            cntr.aktualizujTabele();
            if (!tabela_dane.isEmpty()) {cntr.aktualizujTabele();}
            glowna.getChildren().clear();
            glowna.getChildren().add(pane);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public ObservableList<Czlowieczek> getTabela_dane() {
        return tabela_dane;
    }
}
