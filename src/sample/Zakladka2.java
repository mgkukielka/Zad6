...
public class Zakladka2 {
    ...
    @FXML
    private GridPane gridpane_dane;
    @FXML
    private Button liczStat;

    public void zmienPlansze(ActionEvent actionEvent) {
        parentController.dane(actionEvent);
    }
    ...
    public void dodajCzlowieczka(ActionEvent actionEvent) {
        ...
        if (nowy.sprawdz()) {
            parentController.dodaj(nowy);
         }

    }
    public void aktualizujTable() {
        if (this.parentController.getLista()!=null) {
            this.table.setItems(parentController.getLista());
        }
    }

    public void dodawajDane(){
        if (getParentController().getZalogowany()==null||this.parentController.getZalogowany().getRola() != "admin"){
            this.gridpane_dane.setVisible(false);
            liczStat.setVisible(true);
        } else {
            this.gridpane_dane.setVisible(true);
            liczStat.setVisible(false);
        }
